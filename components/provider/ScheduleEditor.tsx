...
export default function ScheduleEditor({ value, onChange }: ScheduleEditorProps) {
  function addSlot() {
    onChange([...value, { weekday: "Lunes", startTime: "09:00", endTime: "10:00" }]);
  }
  ...
  function updateSlot(index: number, field: keyof ScheduleSlot, val: string) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {value.map((slot, i) => (
          <div key={i} className="flex items-center gap-2 bg-sand rounded-xl px-3 py-2.5 border border-border">
            {/* Día */}
            <select
              value={slot.weekday}
